...
def run_python_file(working_directory, file_path, args=None):
    try:
        ...
        if args:
            command.extend(args)
        completed_process = subprocess.run(command, cwd = working_dir_abs, 
    ...
    parameters=types.Schema(
        ...
        properties={
            "file_path": types.Schema(
                ...
                description="File path, relative to the working directory. For the root directory, use '.'.",
            ),
            "args": types.Schema(
                ...
                description="Command line arguments to pass to a python file.",
            )
        },
        required=["file_path"]
    ),
